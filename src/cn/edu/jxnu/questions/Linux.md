...
系统会默认将错误的信息（STDERR）2重定向到一个名字为1的文件中，而非所想的（STDOUT）中。因此需要加&进行区分。就有了 2>&1 这样的用法：2>&1。

输出日志用的最多的：```sh test.sh > test.log 2>&1 &``` 意思是：执行test脚本，并将标准错误也输出到标准输出当中，最后一个&表示在后台执行。

[参考](https://blog.csdn.net/c19870525/article/details/80756121)|[参考](https://www.cnblogs.com/is-Tina/p/8697299.html)                                                    
